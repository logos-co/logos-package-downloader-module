...
| `getDownloadSource` | `std::string getDownloadSource()` | The transports downloads may use: `"any"` (Logos Storage, then HTTP), `"logos"` (Logos Storage only) or `"http"` (HTTP only) |
| `setDownloadSource` | `LogosMap setDownloadSource(const std::string& source)` | Set and persist the download source. Returns `{success, error?}`; an unknown value is refused. Emits `catalogChanged` when the source changes, since `getCatalog` marks each version the source cannot serve with `sourceAvailable: false` and a `sourceUnavailableReason` |
